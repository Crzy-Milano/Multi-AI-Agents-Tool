...
# Start fully local: python3 multi_agent-11.py --agent ollama tinyllama:latest --agent ollama mistral:7b-instruct-q4_0 --agent ollama mistral:latest --orch ollama mistral:latest
...
def get_client(provider):
    ...
    if local:
        # Local providers do not need an API key
...
def provider_available(provider):
    ...
    if cfg.get("local", False):
        return True  # always available if Ollama/LM Studio is running
...
def handle_command(cmd, agent_cfgs, orch_cfg, mode):
    ...
    if keyword in ("-models", "!models"):
        ...
        if len(parts) < 3:
            ...
            elif PROVIDER_CONFIGS[provider].get("local", False):
                print(c(f"{provider} is a local provider — no API key needed.", YELLOW))
            elif not PROVIDER_CONFIGS[provider].get("env_key"):
                print(c(f"{provider} does not require an API key.", YELLOW))
            else:
...
def interactive_loop(agent_cfgs, orch_cfg, mode):
    print(header("Multi-Agent Orchestrator  v11"))
    print_status(agent_cfgs, orch_cfg, mode)
    print(c("Type -models to switch models, !help for all commands.\n", GRAY))

    while True:
        try:
            prompt = input(c("You  ▶  ", BOLD, GREEN)).strip()
        except (EOFError, KeyboardInterrupt):
            print("\nGoodbye!"); break
...
def main():
    p = argparse.ArgumentParser(
        description="Multi-Agent Orchestrator v11",
...
Starting:
  python3 multi_agent-11.py
  python3 multi_agent-11.py --mode code
  python3 multi_agent-11.py --agent groq llama-3.3-70b-versatile --agent mistral mistral-small-latest --agent together meta-llama/Llama-3.3-70B-Instruct-Turbo-Free
...
""")
    ...
    # Check if at least one key is available (local providers always count)
    any_key = any(
    ...
    if not any_key:
        ...
        print("  python3 multi_agent-11.py --agent ollama llama3.3 --agent ollama mistral --agent ollama phi4")
        sys.exit(1)

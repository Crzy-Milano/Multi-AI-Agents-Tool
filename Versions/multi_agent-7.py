...
# Start fully local: python3 multi_agent-7.py --agent ollama tinyllama:latest --agent ollama mistral:7b-instruct-q4_0 --agent ollama mistral:latest --orch ollama mistral:latest

# ─── Ollama: detect installed models ─────────────────────────────────────────
...
def get_ollama_models():
    ...
    try:
        ...
        for line in lines[1:]:  # skip header
            parts = line.split()
            if parts:
                models.append(parts[0])  # e.g. "mistral:latest"
...
def interactive_loop(agent_cfgs, orch_cfg, mode):
    print(header("Multi-Agent Orchestrator  v7"))
    print_status(agent_cfgs, orch_cfg, mode)
...
Starting:
  python3 multi_agent-7.py
  python3 multi_agent-7.py --mode code
  python3 multi_agent-7.py --agent groq llama-3.3-70b-versatile --agent mistral mistral-small-latest --agent together meta-llama/Llama-3.3-70B-Instruct-Turbo-Free
...
""")
    ...
    if not any_key:
        ...
        print("  python3 multi_agent-7.py --agent ollama llama3.3 --agent ollama mistral --agent ollama phi4")
        sys.exit(1)

...
# Start fully local: python3 multi_agent-8.py --agent ollama tinyllama:latest --agent ollama mistral:7b-instruct-q4_0 --agent ollama mistral:latest --orch ollama mistral:latest

# ─── Local providers: detect installed models ────────────────────────────────
...
def models_menu(agent_cfgs, orch_cfg, mode):
    ...
    while True:
        ...
        if provider == "ollama":
            ...
            print()
            print(c(f"  Installed Ollama models:", BOLD))
        elif provider == "lmstudio":
            models = get_lmstudio_models()
            print()
            print(c(f"  Installed LM Studio models:", BOLD))
        else:
            models = cfg["models"]
            print()
...
def interactive_loop(agent_cfgs, orch_cfg, mode):
    print(header("Multi-Agent Orchestrator  v8"))
    print_status(agent_cfgs, orch_cfg, mode)
...
Starting:
  python3 multi_agent-8.py
  python3 multi_agent-8.py --mode code
  python3 multi_agent-8.py --agent groq llama-3.3-70b-versatile --agent mistral mistral-small-latest --agent together meta-llama/Llama-3.3-70B-Instruct-Turbo-Free
...
""")
    ...
    if not any_key:
        ...
        print("  python3 multi_agent-8.py --agent ollama llama3.3 --agent ollama mistral --agent ollama phi4")
        sys.exit(1)
